...
- **Main Agent Loop**：Pi Agent 的统一入口，可在最多 12 个模型回合内按需读取 Topic 记忆、上下文实体和历史视觉资产。没有后续 Tool Call 的非空普通文本自然成为最终回答；空响应失败关闭。图片生成、编辑或只读分析均由同一 Main Agent 判断；只有图片变更才调用 `generate_image`。
...
- **主题记忆**：每个 Topic 持久化近期对话、滚动摘要、事实、偏好、活动任务和最近引用资产。Main Agent 通过非终止的 `update_conversation_memory` 暂存语义补丁，仅在最终回答或 Main Agent 任务交接成功后提交；失败或取消时丢弃。完整历史保留，但每轮只注入受限快照；不使用跨 Topic 检索或向量数据库。
...
- **图像执行合同**：由 Main Agent 在读取必要上下文和显式锁定的 Skill 后形成；本地只做协议、引用、权限、数量和执行安全校验，不再发起独立 Planner 模型请求。
- **图像交付动作**：会产生或改变交付物的生成、编辑、批量输出和导出操作，必须先经过 Main Agent 的图像执行合同。
- **图像 Skill Snapshot**：每个图片 Turn 在首次 Main Agent sampling 前先注入紧凑 Skill 目录，再以独立用户 `<skill>` 片段依次注入完整 `imagegen` 与显式锁定的视觉 Skill。完整 Skill 受 8KB UTF-8 安全预算约束；截断必须记录告警，不能静默摘要为风格标签。视觉 Skill 的 `renderPrompt` 输出约定由 Main Agent 直接执行，`generate_image.args.prompt` 是唯一最终供应商 Prompt；不启动独立 Image Planner、Prompt Optimizer 或第二个 Prompt 模型。Skill 的工作流、输出检查或质量门不进入供应商请求。
- **Skill 选择与延续**：图片视觉 Skill 只能由 UI `activeSkillId`、`$skill`、Skill 路径或与 manifest 精确匹配的 ID/名称选择，不能按 trigger hint 或语义相似度猜测。普通新 Turn 不继承视觉 Skill；确认、显式恢复、显式失败重试和部分成功补齐继承任务锁定 Skill，并在新尝试重新读取、校验。

图像执行合同校验通过后，本地只做合同、安全、权限、幂等和供应商调用；旧 Planner/Brief/Prompt Compiler 字段只在一次性迁移边界被识别并删除，不能重新编译或覆盖 Prompt。`args.prompt`、图片就绪事件、持久化 trace 和供应商请求必须使用同一文本并记录可比对 hash。供应商能力不足只作为模型配置错误处理，不添加供应商专用 Agent 协议分支。
