...
- **恢复门控**：用户手输新消息且存在恢复候选时，Main Agent 先运行最多两轮的受限 Pi 门控，只能调用 `resolve_failed_task_recovery`；不接收图片、完整历史、Topic 记忆、实体清单或其他工具。点击重试直接提交 `recoveryTaskId`，已知路由时不运行门控。门控只决定恢复、继续当前请求或说明无法恢复，不改写原始 Brief。
- **恢复执行**：聊天和只读视觉重新进入 Main Agent；图片、Planner 和 Skill 失败重新进入 Image Planner 并生成新合同；已保存资产的本地交付失败只重新交付对应资产。部分成功必须先选择补齐缺失项或全部重做。原 Skill 默认保持，手动 Skill 可以覆盖；未知、禁用或被模型替换的 Skill 失败关闭。
- **只读图片对话**：描述、识别、OCR、评价、比较、建议或只输出 Prompt，不改变、导出或生成图片。
- **Image Planner**：接收用户原始需求、必要的原始历史、稳定引用 ID、Main Agent 的结构化 `visualSummary` 和唯一选中 Skill 的精简 manifest，形成生成、编辑、批量、导出或 Skill 执行合同。Planner 不接收图片像素或完整 `SKILL.md`；原始需求仍是权威 Brief，`visualSummary` 只提供可验证的视觉证据。
...
Image Planner 合同校验通过后，本地按合同确定性执行；不会再启动第二个模型 Loop 决定是否调用图像工具。供应商能力不足只作为模型配置错误处理，不添加供应商专用 Agent 协议分支。
