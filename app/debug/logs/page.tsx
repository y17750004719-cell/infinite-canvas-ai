  ...
  return (
    <main className="min-h-screen bg-neutral-950 px-4 py-8 text-neutral-100 sm:px-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="overflow-hidden rounded-[28px] border border-neutral-800 bg-neutral-900/60 backdrop-blur">
          <div className="border-b border-neutral-800 px-6 py-5">
            <p className="text-xs uppercase tracking-[0.35em] text-neutral-400">Local Debug Logs</p>
          ...
          <div className="grid gap-3 border-b border-neutral-800 px-6 py-5 md:grid-cols-3">
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/40 px-4 py-3">
              <p className="text-[11px] uppercase tracking-[0.25em] text-neutral-400">Current Startup</p>
              <p className="mt-2 break-all text-sm text-neutral-100">{startupSession.startupId}</p>
            </div>
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/40 px-4 py-3">
              <p className="text-[11px] uppercase tracking-[0.25em] text-neutral-400">Started At</p>
              <p className="mt-2 text-sm text-neutral-100">{formatTimestamp(startupSession.startedAt)}</p>
            </div>
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/40 px-4 py-3">
              <p className="text-[11px] uppercase tracking-[0.25em] text-neutral-400">Current File</p>
              <p className="mt-2 break-all text-sm text-neutral-100">{currentLogFile}</p>
            </div>
          </div>
        ...
        <section className="rounded-[28px] border border-neutral-800 bg-neutral-900/60 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-medium">结果</h2>
              <p className="mt-1 text-sm text-neutral-400">{entries.length} 条日志</p>
            </div>
            <p className="max-w-xl text-right text-xs text-neutral-500">
          ...
          <div className="mt-6 space-y-4">
            {entries.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-neutral-800 bg-neutral-950/40 px-6 py-12 text-center text-sm text-neutral-400">
                当前筛选条件下还没有日志。
              </div>
            ) : (
              entries.map((entry, index) => (
                <article
                  ...
                  className="overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-950/40"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3 border-b border-neutral-800 px-5 py-4">
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-400">
                        <span className="rounded-full border border-neutral-800 px-2 py-1 uppercase tracking-[0.2em]">{entry.level}</span>
                        <span className="rounded-full border border-neutral-800 px-2 py-1 uppercase tracking-[0.2em]">{entry.source}</span>
                    ...
                    <div className="space-y-1 text-right text-xs text-neutral-500">
                  ...
                  {entry.details ? (
                    <pre className="overflow-x-auto px-5 py-4 text-xs leading-6 text-neutral-200">
                      {renderDetails(entry)}
                    </pre>
                  ) : null}
                </article>
