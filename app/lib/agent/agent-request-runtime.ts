...
import { executeImageRequest } from './image-tool-service.mjs';
...
import { loadTurnState, resolveContinuationTurn } from './thread-turn-service.mjs';
import { NATIVE_AGENT_INSTRUCTIONS } from './native-agent-instructions.mjs';
import { executeNativeBusinessOperation, saveNativeConfirmation, loadNativeConfirmation, claimNativeConfirmation } from './native-business-ledger.mjs';
import {
...
import { classifyAgentFailureCode, createExecutionRecoveryRecord } from './agent-recovery-service.mjs';
...
import { requireOriginalAsset } from './original-asset.mjs';
import {
...
import { readProviderRegistry, effectiveProviderProtocol } from '../provider-config.mjs';
...
import { estimateContextBudget } from './context-window.mjs';
import type { ContextEvent, ContextWindowState } from '../db';
import {
...
import {
  ...
  hashPrompt,
  hasOnlyImageOperationAmbiguity,
...
import { isReferentialShorthand, parseAgentProposalBlock, resolveContextReference } from './context-reference.mjs';
import type {
  AgentContextEntity,
  AgentContextResolution,
  AgentTaskContract,
} from './context-reference.types';
type WorkingContext = {
  version: 1;
...
import type { GeneratedImageHistoryEntry, SessionVisualAsset } from '../db';
...
const eventSinks = new WeakMap<object, ReturnType<typeof createEventSink>>();
...
const persistApprovalSnapshot = (record: ConfirmationRecord | undefined) => {
  ...
  void threadJournalService.updateThreadState(record.sessionId, { pendingApproval: snapshot });
};
const clearApprovalSnapshot = (record: ConfirmationRecord | undefined) => {
  if (record?.sessionId) void threadJournalService.updateThreadState(record.sessionId, { pendingApproval: null });
};
...
function createTodoUpdateExecutor() {
  return async (args: Record<string, unknown>, context: Record<string, any>) => {
    const tools = createTodoTools({
      readThread: async (threadId: string) => threadJournalService.loadThread(threadId),
      appendThreadEvent: threadJournalService.appendThreadEvent,
...
function writeEvent(controller: ReadableStreamDefaultController, event: AgentEvent) {
  ...
  if (!context) return;
  const canonical = projectNativeEvent(event, context) || [];
  if (canonical.length === 0) return;
  const entries = canonical;
  // The event sink owns ordering and journal/SSE mirroring. Keep the legacy
  // pending list only as a flush handle for the surrounding stream lifecycle.
  let sink = eventSinks.get(controller as unknown as object);
  if (!sink) {
    sink = createEventSink({
      journal: threadJournalService,
      streamController: controller,
      encoder,
      onError: () => {
        try {
          controller.enqueue(encoder.encode(`${JSON.stringify({ type: 'error', code: 'journal_write_failed', message: 'Unable to persist agent event' })}\n`));
        } catch { /* Connection is already closed; the journal task remains settled. */ }
      },
    });
    eventSinks.set(controller as unknown as object, sink);
  }
  // The sink is the sole owner of journal/SSE ordering. Keeping this call
  // asynchronous preserves the producer-facing writeEvent API while making
  // replay and live delivery use exactly the same persisted event.
  sink.persistCanonicalEvents({ events: entries });
}

const handleManagementCommand = (threadId: string, command: { name: string; args: string; raw: string }) => handleManagedCommand(threadId, command);
...
export async function handlePost(request: NextRequest) {
  ...
  try {
    const thread = await loadTurnState(sessionId);
    ...
    if (body.recoveryTaskId || body.clarificationState || body.confirmation) {
      ...
      try {
        journalTurnId = resolveContinuationTurn({ state: thread.state, operationId: operation });
      } catch (error) {
        return NextResponse.json({ error: error instanceof Error ? error.message : 'Continuation is stale', code: 'stale_operation' }, { status: 409 });
      }
    }
  } catch (error) {
  ...
  if (!preparedContext.ok) {
    return NextResponse.json(preparedContext.response.payload, { status: preparedContext.response.status });
  }
  const preparedSnapshot = preparedContext.value;
  const {
    conversationIntent,
    contextEntities: preparedContextEntities,
    sessionVisualAssets,
    contextEvents,
    contextAuditEvents,
    contextModelEvents,
    incomingHistoryRevision,
    incomingUserMessageRevision,
    incomingActiveWindowRevision,
    requestHistoryRevision,
    requestUserMessageRevision,
    requestActiveWindowRevision,
    persistedCompactedWindows,
    nextContextSequence: preparedNextContextSequence,
    replayContext,
    generatedImageHistory,
    knownContextEntityIds,
    knownVisualReferenceIds,
    recentFailedTask,
    selectedContextEntityIds,
    initialBriefSource,
    rawUserCountResolution,
    briefCountResolution,
    explicitBatchCountResolution,
    rawUserDeliveryPlan,
    briefDeliveryPlan,
    initialDeliveryPlan,
    ...
    requestedIntent,
    hasExplicitChatSelection,
    resolvedChatSelection,
    resolvedChatProvider,
    resolvedChatModelMetadata,
  } = preparedSnapshot as any;
  const contextEntities = preparedContextEntities as AgentContextEntity[];
  let nextContextSequence = preparedNextContextSequence;
    ...
    async start(controller) {
      ...
      const takeDurableRunInputs = async (delivery: 'steer' | 'follow_up') => {
        ...
        if (messages.length > 0) {
          await threadJournalService.consumeThreadInputs(sessionId, { threadId: sessionId, turnId: journalTurnId, taskId, operationId, runId }, delivery);
        }
        return messages;
      };
      ...
      const generateImagePayload = async (
        ...
        await eventSinks.get(controller as unknown as object)?.flush();
        ...
        const {
          selection: resolvedImageSelection,
          provider: resolvedProvider,
          allowedModelIds,
        } = resolveImageExecutionSelection({
          providers,
        ...
        try {
          taskResults = await settleCanvasImageGenerationRequests({
            ...
            const generationResponse = await executeImageRequest(generationRequest);
          ...
          const executeNativeTurn = async () => {
            mainAgentRequestCount += 1;
              ...
              return await runAgentTurn({
                startKeepalive: startMainAgentKeepalive,
                request: {
                sessionId,
                ...
                executeTool: async (toolName, args, context) => {
                  ...
                  return dispatchRegisteredApplicationTool({
                    name: toolName,
                    args,
                    allowedTools: resolveMainAgentToolNames(),
                    registry: mainAgentRegistry,
                    executionContext: {
                      allowedTools: resolveMainAgentToolNames(),
                ...
                hasImageResult: Boolean(nativeGeneratedImageResult),
                toolCallCount: toolCallRecords.length,
                },
              });
          };
          loopResult = await executeAgentTurn({ execute: executeNativeTurn });
        }
