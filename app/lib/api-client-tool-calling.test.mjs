...
const geminiSource = fs.readFileSync(path.resolve(import.meta.dirname, 'gemini-chat-transport.mjs'), 'utf8');
...
test('gemini chat maps function declarations calls and responses', () => {
  assert.match(source, /convertGeminiMessages/);
  assert.match(geminiSource, /functionCall/);
  assert.match(geminiSource, /functionResponse/);
  assert.match(source, /toolConfig/);
  assert.match(geminiSource, /pendingToolResponses/);
  assert.match(geminiSource, /mode: 'ANY'/);
  assert.match(geminiSource, /allowedFunctionNames: \[toolChoice\.function\.name\]/);
  assert.match(source, /resolveGeminiFunctionCallingConfig\(request\.toolChoice\)/);
  assert.match(source, /thoughtSignature\?: string/);
  assert.match(geminiSource, /extractGeminiToolCalls/);
  assert.match(geminiSource, /geminiToolCallToPart/);
  assert.match(source, /geminiParts\?: GeminiContentPart\[\]/);
  assert.match(source, /geminiSourceModel\?: string/);
  assert.match(geminiSource, /functionResponse:[\s\S]{0,320}isSyntheticGeminiToolCallId/);
  assert.match(geminiSource, /replayGeminiParts\(message\.geminiParts/);
...
test('Gemini chat drops empty messages and OpenAI compatibility retries only a missing flat tool choice', () => {
  assert.match(geminiSource, /if \(parts\.length\) contents\.push/);
